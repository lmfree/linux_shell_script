...
if len(sys.argv) < 3:
	print 'please input: ', sys.argv[0], 'dns_name(ip) port' 
	exit(0) 
# 端口映射配置信息
#CFG_REMOTE_IP = 'www.gogfw.ga'
CFG_REMOTE_IP = sys.argv[1]
#CFG_REMOTE_PORT = 2851
CFG_REMOTE_PORT = int(sys.argv[2])
CFG_LOCAL_IP = '0.0.0.0'
CFG_LOCAL_PORT = 3000

print 'forward data to:',CFG_REMOTE_IP,':',CFG_REMOTE_PORT
#exit(0)
# 接收数据缓存大小
PKT_BUFF_SIZE = 2048
...
def tcp_mapping(remote_ip, remote_port, local_ip, local_port):
  ...
  #micahlee
  #local_server.settimeout(CHECK_TIMEOUT)
